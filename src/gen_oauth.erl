...
http_stream_fun(ServerRef, Url, Params) ->
    gen_fsm:sync_send_event(ServerRef, {http_stream_fun, Url, Params}).
...
authenticated({http_stream_fun, Url, Params}, _From,
              #state{consumer=Consumer, access_token=AccessToken,
                     access_secret=AccessSecret}=State) ->
    StreamFun = oauth_post_stream_fun(Url, Params, Consumer,
                                      AccessToken, AccessSecret),
    {reply, {ok, StreamFun}, authenticated, State}.
...
%% Params = [{K,V}]
oauth_post_stream_fun(Url, Params, Consumer, Token, TokenSecret) ->
    fun() ->
